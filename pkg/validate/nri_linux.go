...
var _ = framework.KubeDescribe("NRI", func() {
	...
	Context("plugin error handling", Serial, func() {
		...
		It(
			"should propagate RunPodSandbox plugin error, clean up resources, and allow immediate retry",
			func(ctx SpecContext) {
				...
				retryPodID, retryErr := rc.RunPodSandbox(
					ctx,
					podConfig,
					framework.TestContext.RuntimeHandler,
				)
		...
		It(
			"should propagate CreateContainer plugin error and allow immediate retry",
			func(ctx SpecContext) {
				...
				retryCtrID, retryErr := rc.CreateContainer(
					ctx,
					podID,
					retryContainerConfig,
					podConfig,
				)
				Expect(retryErr).NotTo(HaveOccurred(),
